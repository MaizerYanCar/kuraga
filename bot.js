...
client.login(ODMzNjY4MjU5NzcyNjI5MDMy.YH1sEQ.lnhHz3wz0eOEU_DIbv9Huy1YV0Q);
...
client.on("message", message => {
    if(message.content.toLowerCase() == "why") 
    return message.channel.send(`Why adamdır :heart: :heart:`)
});
...
db.delete(`cezali_${member.guild.id + member.id}`)
    ...
    member.roles.remove('831559473867915325');
  }, ms(sürejail));
}
})
...
db.delete(`muteli_${member.guild.id + member.id}`)
    ...
    member.roles.remove('815971195668398132');
  }, ms(süre));
}
})
...
var randomMesajlar = ["Mαîzer ❤️ Why"]
